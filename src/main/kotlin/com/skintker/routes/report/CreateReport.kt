...
fun Route.createReport() {
    ...
    val databaseManager by inject<DatabaseManagerImpl>()
    /**
    ...
    put("/report/{$TOKEN_PATH_PARAM}") {
        try {
            ...
            val customer = call.receive<DailyLog>()
            if (token.isNullOrEmpty()) {
                throw TokenException()
            }
            databaseManager.saveReport(token, customer)
            ...
            when (exception) {
                ...
                else -> call.respondText(GENERIC_ERROR_RESPONSE, status = HttpStatusCode.BadRequest)
            }
        }
    }
}
